...
import { LottoStatus, LottoTicket, LottoTicketClaimData } from 'config/constants/types'
import { LottoUserGraphEntity, LottoRoundGraphEntity } from 'state/types'
import { multicallv2 } from 'utils/multicall'
import LottoV2Abi from 'config/abi/LottoV2.json'
import { NUM_ROUNDS_TO_CHECK_FOR_REWARDS } from 'config/constants/Lotto'
...
interface RoundDataAndUserTickets {
  roundId: string
  userTickets: LottoTicket[]
  finalNumber: string
}

const LottoAddress = getLottoV2Address()

const fetchCakeRewardsForTickets = async (
  winningTickets: LottoTicket[],
): Promise<{ ticketsWithUnclaimedRewards: LottoTicket[]; cakeTotal: BigNumber }> => {
  ...
  try {
    const cakeRewards = await multicallv2({ abi: LottoV2Abi, calls })
...
const getWinningNumbersForRound = (targetRoundId: string, lotteriesData: LottoRoundGraphEntity[]) => {
  const targetRound = lotteriesData.find((pastLotto) => pastLotto.id === targetRoundId)
...
const fetchUnclaimedUserRewards = async (
  ...
  // If the web3 provider account doesn't equal the userLottoData account, return an empty array - this is effectively a loading state as the user switches accounts
  if (userLottoData.account.toLowerCase() !== account.toLowerCase()) {
    return []
  }

  // Filter out rounds without subgraph data (i.e. >100 rounds ago)
  const roundsInRange = rounds.filter((round) => {
    const lastCheckableRoundId = parseInt(currentLottoId, 10) - MAX_LOTTERIES_REQUEST_SIZE
    const roundId = parseInt(round.LottoId, 10)
  ...
  if (roundsToCheck.length > 0) {
    const idsToCheck = roundsToCheck.map((round) => round.LottoId)
