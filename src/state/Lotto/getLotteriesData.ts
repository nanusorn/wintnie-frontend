...
import { LottoRoundGraphEntity, LottoResponse } from 'state/types'
...
const applyNodeDataToLotteriesGraphResponse = (
  nodeData: LottoResponse[],
  graphResponse: LottoRoundGraphEntity[],
): LottoRoundGraphEntity[] => {
  ...
  const nodeRoundsWithGraphData = nodeData.map((nodeRoundData) => {
    const graphRoundData = graphResponse.find((graphResponseRound) => graphResponseRound.id === nodeRoundData.LottoId)
    return {
...
): Promise<LottoRoundGraphEntity[]> => {
  try {
    const response = await request(
      GRAPH_API_Lotto,
      gql`
        query getLotteries($first: Int!, $skip: Int!, $where: Lotto_filter) {
...
const getLotteriesData = async (currentLottoId: string): Promise<LottoRoundGraphEntity[]> => {
  const idsForNodesCall = getRoundIdsArray(currentLottoId)
