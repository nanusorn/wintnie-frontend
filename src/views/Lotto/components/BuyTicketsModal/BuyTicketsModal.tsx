...
import { useCake, useLottoV2Contract } from 'hooks/useContract'
import useTheme from 'hooks/useTheme'
import useTokenBalance from 'hooks/useTokenBalance'
import { useCallback, useEffect, useMemo, useState } from 'react'
import { useAppDispatch } from 'state'
import { usePriceCakeBusd } from 'state/farms/hooks'
import { fetchUserTicketsAndLotteries } from 'state/Lotto'
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        return requiresApproval(cakeContractReader, account, LottoContract.address)
      },
      onApprove: () => {
        return callWithMarketGasPrice(cakeContractApprover, 'approve', [LottoContract.address, MaxUint256])
      },
      ...
      onConfirm: () => {
        ...
        return callWithMarketGasPrice(LottoContract, 'buyTickets', [currentLottoId, ticketsForPurchase])
      },
      onSuccess: async ({ receipt }) => {
        onDismiss?.()
        dispatch(fetchUserTicketsAndLotteries({ account, currentLottoId }))
        toastSuccess(t('Lotto tickets purchased!'), <ToastDescriptionWithTx txHash={receipt.transactionHash} />)
      },
    })
