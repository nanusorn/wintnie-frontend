...
import { useLottoV2Contract } from 'hooks/useContract'
...
const ClaimInnerContainer: React.FC<React.PropsWithChildren<ClaimInnerProps>> = ({ onSuccess, roundsToClaim }) => {
  ...
  const { maxNumberTicketsPerBuyOrClaim, currentLottoId } = useLotto()
  ...
  const LottoContract = useLottoV2Contract()
  ...
  const parseUnclaimedTicketDataForClaimCall = (ticketsWithUnclaimedRewards: LottoTicket[], LottoId: string) => {
    ...
    return { LottoId, ticketIds, brackets }
  }
  ...
  const handleClaim = async () => {
    const { LottoId, ticketIds, brackets } = claimTicketsCallData
    const receipt = await fetchWithCatchTxError(() => {
      return callWithEstimateGas(LottoContract, 'claimTickets', [LottoId, ticketIds, brackets])
    })
    if (receipt?.status) {
      toastSuccess(
        t('Prizes Collected!'),
        <ToastDescriptionWithTx txHash={receipt.transactionHash}>
          {t('Your CAKE prizes for round %LottoId% have been sent to your wallet', { LottoId })}
  ...
  const handleBatchClaim = async () => {
    ...
    for (const ticketBatch of ticketBatches) {
      ...
      const receipt = await fetchWithCatchTxError(() => {
        return callWithEstimateGas(LottoContract, 'claimTickets', [
          LottoId,
      ...
      if (receipt?.status) {
        ...
        if (receipts.length !== transactionsToFire) {
          toastSuccess(
            ...
            <ToastDescriptionWithTx txHash={receipt.transactionHash}>
              {t(
                'Claim %claimNum% of %claimTotal% for round %LottoId% was successful. Please confirm the next transaction',
                {
