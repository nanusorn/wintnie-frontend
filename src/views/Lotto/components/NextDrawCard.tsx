...
const NextDrawCard = () => {
  ...
  const { currentLottoId, isTransitioning, currentRound } = useLotto()
  const { endTime, amountCollectedInCake, userTickets, status } = currentRound

  const [onPresentViewTicketsModal] = useModal(<ViewTicketsModal roundId={currentLottoId} roundStatus={status} />)
  ...
  const isLottoOpen = status === LottoStatus.OPEN
  const userTicketCount = userTickets?.tickets?.length || 0

  const getPrizeBalances = () => {
    if (status === LottoStatus.CLOSE || status === LottoStatus.CLAIMABLE) {
      return (
  ...
  const getNextDrawId = () => {
    if (status === LottoStatus.OPEN) {
      return `${currentLottoId} |`
    }
    if (status === LottoStatus.PENDING) {
      return ''
    }
    return parseInt(currentLottoId, 10) + 1
  }
    ...
    <StyledCard>
      <CardHeader p="16px 24px">
        <Flex justifyContent="space-between">
          ...
          <Text>
            {currentLottoId && `#${getNextDrawId()}`} {Boolean(endTime) && getNextDrawDateTime()}
          </Text>
        </Flex>
      </CardHeader>
      <CardBody>
      ...
      <CardFooter p="0">
        {isExpanded && (
          <NextDrawWrapper>
            <RewardBrackets LottoNodeData={currentRound} />
          </NextDrawWrapper>
        )}
        {(status === LottoStatus.OPEN || status === LottoStatus.CLOSE) && (
