...
import { LottoTicket, LottoTicketClaimData } from 'config/constants/types'
import { fetchLotto } from 'state/Lotto/helpers'
import { getWinningTickets } from 'state/Lotto/fetchUnclaimedUserRewards'
import { fetchUserTicketsForOneRound } from 'state/Lotto/getUserTicketsData'
import { LottoRound } from 'state/types'
import { useTranslation } from '@pancakeswap/localization'
import useTheme from 'hooks/useTheme'
import orderBy from 'lodash/orderBy'
import WinningNumbers from '../WinningNumbers'
import { processLottoResponse } from '../../helpers'
...
const PreviousRoundTicketsInner: React.FC<React.PropsWithChildren<{ roundId: string }>> = ({ roundId }) => {
  const [LottoInfo, setLottoInfo] = useState<LottoRound>(null)
  const [allUserTickets, setAllUserTickets] = useState<LottoTicket[]>(null)
  const [userWinningTickets, setUserWinningTickets] = useState<{
    allWinningTickets: LottoTicket[]
    ticketsWithUnclaimedRewards: LottoTicket[]
    isFetched: boolean
    claimData: LottoTicketClaimData
  ...
  useEffect(() => {
    const addWinningTicketInfoToAllTickets = (
      _allTickets: LottoTicket[],
      _allWinningTickets: LottoTicket[],
    ): LottoTicket[] => {
    ...
    const fetchData = async () => {
      const [userTickets, LottoData] = await Promise.all([
      ...
      const winningTickets = await getWinningTickets({
        roundId,
        userTickets,
        finalNumber: processedLottoData.finalNumber.toString(),
      })
      ...
      setLottoInfo(processedLottoData)
  ...
  return (
    <>
      ...
      <TopBox>
        ...
        {LottoInfo?.finalNumber ? (
          <WinningNumbers number={LottoInfo.finalNumber.toString()} />
        ) : (
